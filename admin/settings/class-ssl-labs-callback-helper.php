...
/**
 * SSL_Labs Callback Helper Class
 *
 ...
 */
class SSL_Labs_Callback_Helper {
	...
	private function get_id_attribute( $id ) {
		return ' id="ssl_quality_checker_settings[' . $id . ']" ';
	}

	private function get_name_attribute( $name ) {
		return ' name="ssl_quality_checker_settings[' . $name . ']" ';
	}
	...
	private function get_label_for( $id, $desc ) {
		return '<label for="ssl_quality_checker_settings[' . $id . ']"> '  . $desc . '</label>';
	}
	...
	public function checkbox_callback( $args ) {
		...
		$value = SSL_Labs_Option::get_option( $args['id'] );
	...
	public function multicheck_callback( $args ) {
		...
		$old_values = SSL_Labs_Option::get_option( $args['id'], array() );
	...
	public function radio_callback( $args ) {
		...
		$old_value = SSL_Labs_Option::get_option( $args['id'] );
	...
	private function input_type_callback( $type, $args ) {
		...
		$value = SSL_Labs_Option::get_option( $args['id'], $args['std']  );
	...
	public function rich_editor_callback( $args ) {
		...
		if ( $wp_version >= 3.3 && function_exists( 'wp_editor' ) ) {
			ob_start();
			wp_editor( stripslashes( $value ), 'ssl_quality_checker_settings_' . $args['id'], array( 'textarea_name' => 'ssl_quality_checker_settings[' . $args['id'] . ']' ) );
			$html = ob_get_clean();
		} else {
	...
	public function upload_callback( $args ) {
		...
		$html .= '<span>&nbsp;<input type="button" class="' .  'ssl_quality_checker_settings_upload_button button-secondary" value="' . __( 'Upload File', $this->plugin_name ) . '"/></span>';
